...
"The fact entry could not be created. #{@fact.errors.full_messages.to_sentence}" },
  ...
  def update
    ...
    if @fact.update(fact_params)
      ...
      render json: { error:
    "Unable to update the fact: #{@fact.errors.full_messages.to_sentence}" },
             status: 400
    end
  end
  ...
  def destroy
    # your code goes here
    @fact.destroy
    render json: { message: 'Fact record succesfully deleted' }, status: 200
  end
  ...
  def check_access
    @member = Member.find(params[:member_id])
    return unless @member.user_id != current_user.id

    render json: { message: 'The current user is not authorized for that data.' }, status: :unauthorized
  end
end

# cQcc2C58u0yq8pAZ22_FFgmXR26gMH4pRzqR1uGWgpCUNVHaK2LvDSDek2qsdCZkNketnWhBLv789pjHnUZKJA
